...
export default function TelaDetalhesOcorrencia() {
    ...
    const [idOcorrencia, setIdOcorrencia] = useState(route.params?.idOcorrencia);
    ...
    useEffect(() => {
      ...
      const fetchEnvolvidos = async () => {
        try {
          const response = await axios.get(`http://localhost/api-easy_search/verificaEnvolvidos.php?idOcorrencia=${idOcorrencia}`);
    ...
    useEffect(() => {
      const fetchOcorrencia = async () => {
        try {
          const response = await axios.get(`http://localhost/api-easy_search/ocorrencias.php?idOcorrencia=${idOcorrencia}`);
    ...
    useEffect(() => {
      ...
      const fetchDeclarante = async () => {
        try {
          const response = await axios.get(`http://localhost/api-easy_search/declarante.php?idOcorrencia=${idOcorrencia}`);
          setDeclarante(response.data);
        } catch (error) {
          console.error(error);
        }
      };
        ...
        <Animatable.View animation="fadeInDown" delay={250}>
            ...
            {carregando ? (
              ...
              <FlatList
                ...
                keyExtractor={(item) => `${item.cidadaoId}-${item.idOcorrencia}`}
                renderItem={({ item }) => (
                  <View style={styles.lista} key={`${item.cidadaoId}-${item.idOcorrencia}`}>
                    <Text style={styles.apresentarDadosOcorrencia}>ID da Ocorrencia: {item.idOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Descrição da Ocorrencia: {item.descricaoOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Tipo da Ocorrencia: {item.nomeOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Data da Ocorrencia: {item.dataOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Hora da Ocorrencia: {item.horaOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Objetos Envolvidos: {item.objetosOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>CEP da Ocorrencia: {item.cepOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Relato da Ocorrencia: {item.relatoOcorrencia}</Text>
                    <Text style={styles.apresentarDadosOcorrencia}>Nome do Autuante: {item.nomeCidadao} {item.sobrenomeCidadao}</Text>
                    <View style={styles.containerDados}>
                      <Text style={styles.apresentarDadosOcorrencia}>CPF do Autuante: </Text>
                      <TextInputMask
                        style={styles.apresentarDadosOcorrencia}
                        type={'cpf'}
